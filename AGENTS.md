...
0 章は他プロジェクトにも転用可能な共通ルール、1 章以降は本プロジェクト固有の指示です。
...
- 言語
  - 指定がなければ日本語で応答する
- 心構え
  - ユーザーの理解を優先し、思い込みを避けて複数案を検討する
  - 曖昧な点は必ず確認する
- 学びメモ
  - 作業後の学び・改善案は `improvement-notes.md` に簡潔に記録し、ガイド改変は合意後に行う
- 範囲
  - ユーザー要求の範囲で最小限に解決し、不要な機能や複雑さを足さない

### 0.2 作業プロセスと品質
- 進め方
  - 複数ステップなら簡潔にプラン共有 → 実行
  - 編集前にファイル全体を把握し、変更理由と検証手順を残す
- 設計・実装
  - 単一責務（目安 300 行未満）を意識し、既存のパターン/命名/フォーマットに合わせる
  - クリーンでモジュール化し、過剰設計を避ける
  - 依存追加は軽量で理由を示す
- 品質・安全
  - セキュリティ脆弱性・明らかなバグ・非効率な実装は見つけ次第指摘する
  - 修正は「失敗テストの再現 → 最小修正 → 既存テスト確認」の順で進める
  - エラーは適切に処理し、自明でない処理に短いコメントを添える
- テスト
  - テストがなければ追加を検討し、既存テストは必ず通す
  - 変更後の検証手順を記録する
- 形式
...
### 0.3 運用・安全
- Git/CI
  - ブランチ切替は `switch`、復元は `restore` を推奨
  - 作業は `dev` 配下のトピックブランチで行い、`main` へ直接 push しない
  - main は強制 push 禁止の保護ブランチとする
  - `dev` を定期的に rebase/merge し、差分を小さく保つ
  - PR は最低 1 名のレビューを通す
  - PR・Issue の操作は必要に応じて CLI ツール（例: gh, glab）も利用してよい
- 制約
  - 提案の実装は承認後に行う
  - 重いビルド/テストや DB・本番データ変更は指示がなければ実施しない
  - 秘密情報をログに残さない
- 環境
  - リポジトリは `~/dev/github.com/<ユーザー名>/<リポジトリ名>` 配下で管理する想定
