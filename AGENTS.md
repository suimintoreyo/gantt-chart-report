...
- 学びメモ: 作業後の学び・改善案は `improvement-notes.md` に簡潔に記録し、ガイドの改変は合意後に行う。
- 作業プロセス: 複数ステップなら簡潔にプラン共有 → 実行。編集前にファイル全体を把握し、変更理由と検証手順を残す。テストがなければ追加を検討。
- コード品質: 既存パターン/命名/フォーマットに合わせ、単一責務（目安 300 行未満）で過剰設計を避ける。エラーは適切に処理し、依存追加は軽量・妥当な理由を伴う。
- ドキュメント: 自明でない処理に短いコメント。可能ならファイル冒頭に「パス/役割/存在理由/関連ファイル」を記すヘッダーを置き、既存ヘッダーは残す。
- ファイル形式: UTF-8（BOM なし）、改行は LF。
...
- 出力スタイル: したこと/必要なこと/次のステップを簡潔に、リスクや代替案はトレードオフ付きで共有。パスは `path/to/file:line` 形式。長文を避け箇条書きを活用。
